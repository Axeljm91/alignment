 ...
 * Unique id of a layer
 */
typedef unsigned int PbsLayerId_SC;

/**
 * Element type.
 */
enum PBS_ELEMENT_TYPE_SC {
  PBS_ELEMENT_SLEW_SEGMENT_SC = 0,
  PBS_ELEMENT_DOSE_POINT_SC   = 1,
  PBS_ELEMENT_DOSE_SEGMENT_SC = 2
};

/**
 * Element of a layer.
 */
struct ScanningControllerPbsLayerElement_SC {
  PBS_ELEMENT_TYPE_SC type;  /* Type of element */
...
struct ScanningControllerPbsLayer {
  PbsLayerId_SC id;      /* Unique id of the layer */
  ...
  double kFactor;
  ScanningControllerPbsLayerElement_SC elements<>;
};
...
enum A500ControllerThresholdType {
  ...
  SC_CTRL_RPC_X_FIELD = 9,                                             /* X field */
  SC_CTRL_RPC_Y_FIELD = 10,                                            /* Y field */
  SC_CTRL_RPC_BEAM_CURRENT_FB = 11,                                    /* ISEU beam current */
  SC_CTRL_RPC_MAX_CHARGE_PRIM = 12,                                    /* Primary charge maximum */
  SC_CTRL_RPC_MIN_CHARGE_PRIM = 13,                                    /* Primary charge minimum */
  SC_CTRL_RPC_DOSE_RATE_PRIM = 14,                                     /* Primary dose rate */
  SC_CTRL_RPC_MAX_CHARGE_SEC = 15,                                     /* Secondary charge maximum */
  SC_CTRL_RPC_MIN_CHARGE_SEC = 16,                                     /* Secondary charge minimum */
  SC_CTRL_RPC_DOSE_RATE_SEC = 17,                                      /* Secondary dose rate */
  SC_CTRL_RPC_MAX_CHARGE_TER = 18,                                     /* Tertiary charge maximum */
  SC_CTRL_RPC_MIN_CHARGE_TER = 19,                                     /* Tertiary charge minimum */
  SC_CTRL_RPC_DOSE_RATE_TER = 20,                                      /* Tertiary dose rate */
  SC_CTRL_RPC_X_POS = 21,                                              /* X position */
  SC_CTRL_RPC_X_WIDTH = 22,                                            /* X width */
  SC_CTRL_RPC_Y_POS = 23,                                              /* X position */
  SC_CTRL_RPC_Y_WIDTH = 24,                                            /* Y width */
  SC_CTRL_RPC_THRESHOLD_COUNT = 25                                     /* Count of thresholds */
};
...
struct ScanningControllerBeamData_SC {
   int elementId;         /* unit: - */
   float xDose;           /* unit: [C]  */
   float yDose;           /* unit: [C]  */
...
struct ScanningControllerBeamDataAtNozzleEntrance_SC {
   int elementId;             /* unit: - */
   float padDoseTotal;        /* unit: [C]  */
   float padDoseCenter;       /* unit: [C]  */
   float padDoseQuadrant[4];  /* unit: [C]  */
   float padRatio;            /* unit: -    */
   float xDose;               /* unit: [C]  */
   float yDose;               /* unit: [C]  */
   float xPosition;           /* unit: [mm] */
   ...
   float xWidth;              /* unit: [mm] */
   float yWidth;              /* unit: [mm] */
   float xChannels[12];       /* unit: [C]  */
   float yChannels[12];       /* unit: [C]  */
   int xValid;                /* xPosition and xWidth valid: 0=false, 1=true */
   int yValid;                /* yPosition and yWidth valid: 0=false, 1=true */
};
...
struct ScanningControllerNecuOffsets {
  float padCenter;           /* unit: [C]  */
  float padQuadrant[4];      /* unit: [C]  */
  float xChannels[12];       /* unit: [C]  */
  float yChannels[12];       /* unit: [C]  */
};
...
program RPC_PROGRAM {
  ...
  version RPC_VERSION {
     ...
     * @param layer data.
     */
    ScanningControllerRpcStatus scanningControllerRpcPrepare(ScanningControllerRpcPrepareLayer_in ) = 1;
     ...
     */
    ScanningControllerRpcStatus scanningControllerRpcPrepareWithFile(ScanningControllerRpcPrepareLayerWithFile_in ) = 2;
    ...
    /**
     * Get the average position of the last spot submap.
     *  
     */
    ScanningControllerBeamPosition scanningControllerGetLastSpotPosition(void) = 10;

    /**
     * Get data from the last spot.
     * Precondition: Scanning Controller functional state == SC_CTRL_RPC_IDLE
     */
    ScanningControllerBeamData_SC scanningControllerGetLastElementSpotData(void) = 11;

    /**
     * Get data from the last spot at nozzle entrance.
     * Precondition: Scanning Controller functional state == SC_CTRL_RPC_IDLE
     */
    ScanningControllerBeamDataAtNozzleEntrance_SC scanningControllerGetLastSpotDataAtNozzleEntrance(void) = 12;
