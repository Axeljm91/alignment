...
/**
 * Represents a collection of {@link org.jfree.data.xy.XYSeries} objects that can be used as a
 * dataset.
 */
public class GaussianDataset extends AbstractIntervalXYDataset
     ...
     */
    public GaussianDataset(XYSeries series) {
...
//            series.addVetoableChangeListener(this);
    ...
    /**
     * Adds a series to the collection and sends a {@link org.jfree.data.general.DatasetChangeEvent}
     * to all registered listeners.
     *
     * @param seriesKey  the series (<code>null</code> not permitted).
     *
     * @throws IllegalArgumentException if the key for the series is null or
     *     not unique within the dataset.
     */
    public void addSeries(Comparable seriesKey, double height, double mean, double std) {
        ParamChecks.nullNotPermitted(seriesKey, "series");
        if (getSeriesIndex(seriesKey) >= 0) {
            throw new IllegalArgumentException(
                    "This dataset already contains a series with the key "
                            + seriesKey);
        }
        GaussianFunction2D gauss = new GaussianFunction2D(height, mean, std);
        XYSeries series = sampleFunction2DToSeries(gauss, -26.3, 26.3, 1000, seriesKey);
  ...
  //      series.addVetoableChangeListener(this);
     ...
     *     not unique within the dataset.
     */
    public void updateSeries(Comparable seriesKey, double height, double mean, double std) {
        ParamChecks.nullNotPermitted(seriesKey, "series");
        int index = getSeriesIndex(seriesKey);
        if (index >= 0) {
            GaussianFunction2D gauss = new GaussianFunction2D(height, mean, std);
            XYSeries series = sampleFunction2DToSeries(gauss, -26.3, 26.3, 1000, seriesKey);
  ...
  //          series.addVetoableChangeListener(this);
     ...
     * to all registered listeners.
     *
     * @param index  the series (<code>null</code> not permitted).
     *
     * @throws IllegalArgumentException if the key for the series is null or
     *     not unique within the dataset.
     */
    public void updateSeries(int index, double height, double mean, double std) {
        if (index >= 0) {
            GaussianFunction2D gauss = new GaussianFunction2D(height, mean, std);
            XYSeries series = sampleFunction2DToSeries(gauss, -26.3, 26.3, 1000, getSeriesKey(index));
  ...
  //          series.addVetoableChangeListener(this);
     ...
     *     not unique within the dataset.
     */
    public void updateSeries(int index, Gaussian gauss) {
        if (index >= 0) {
//            GaussianFunction2D gauss2D = new GaussianFunction2D(gauss.getAmplitude(), gauss.getCentroid(), gauss.getSigma());
//            XYSeries series = sampleFunction2DToSeries(gauss2D, -26.3, 26.3, 1000, getSeriesKey(index));
            XYSeries series = sampleGaussianToSeries(gauss, -26.3, 26.3, 1000, getSeriesKey(index));
  ...
  //          series.addVetoableChangeListener(this);
    ...
    /**
     * Creates an {@link org.jfree.data.xy.XYSeries} by sampling the specified function over a
     * fixed range.
     *
     ...
     * @since 1.0.13
     */
    private static XYSeries sampleFunction2DToSeries(Function2D f,
     ...
     * fixed range.
     *
     * @param gauss  the Gaussian function (<code>null</code> not permitted).
     * @param start  the start value for the range.
     * @param end  the end value for the range.
     * @param samples  the number of sample points (must be &gt; 1).
     * @param seriesKey  the key to give the resulting series
     ...
     * @since 1.0.13
     */
    private static XYSeries sampleGaussianToSeries(Gaussian gauss,
                                                     double start, double end, int samples, Comparable seriesKey) {

        ParamChecks.nullNotPermitted(gauss, "Gaussian");
        ParamChecks.nullNotPermitted(seriesKey, "seriesKey");
        if (start >= end) {
            throw new IllegalArgumentException("Requires 'start' < 'end'.");
        }
        ...
        for (int i = 0; i < samples; i++) {
            double x = start + (step * i);
            series.add(x, gauss.getValueAt(x));
        }
        return series;
    }
   ...
   //       series.removeVetoableChangeListener(this);
     ...
     *
     * @throws org.jfree.data.UnknownKeyException if <code>key</code> is not found in the
     *         collection.
     *
     * @since 1.0.9
     */
    ...
    public boolean equals(Object obj) {
        ...
        if (!(obj instanceof GaussianDataset)) {
            return false;
        }
        GaussianDataset that = (GaussianDataset) obj;
    ...
    public Object clone() throws CloneNotSupportedException {
        GaussianDataset clone = (GaussianDataset) super.clone();
     ...
     * x-values, if/when the dataset is used as an {@link org.jfree.data.xy.IntervalXYDataset}.
     *
    ...
    /**
     * Sets the interval width and sends a {@link org.jfree.data.general.DatasetChangeEvent} to all
